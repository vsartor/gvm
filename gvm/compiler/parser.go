...
func parseInstruction(repr string, ctxt gvm.Context) gvm.Code {
	...
	if err != nil {
		gvm.Logger.Criticalf("l%d: Parsing instruction: %s\n", ctxt.LineNum, err.Error())
		os.Exit(1)
	}
...
func parseRegister(repr string, ctxt gvm.Context) gvm.Code {
	if repr[0] != 'r' {
		gvm.Logger.Criticalf("l%d: Parsing register: Expected 'r', got '%c'.\n", ctxt.LineNum, repr[0])
		os.Exit(1)
	}

	reg, err := strconv.ParseInt(repr[1:], 10, 64)
	if err != nil {
		gvm.Logger.Criticalf("l%d: Parsing register: Expected integer but got '%s'.\n", ctxt.LineNum, repr[1:])
		os.Exit(1)
	}
...
func parseInt(repr string, ctxt gvm.Context) gvm.Code {
	...
	if err != nil {
		gvm.Logger.Criticalf("l%d: Parsing integer: Expected integer but got '%s'.\n", ctxt.LineNum, repr[1:])
		os.Exit(1)
	}
